...
``AddAuthentication`` adds the authentication services to DI and configures ``"Bearer"`` as the default scheme. The Api now recognizes the IdentityServer token service as the ``Authority``, requires communication through ``http`` and presents itself as ``api1``.
